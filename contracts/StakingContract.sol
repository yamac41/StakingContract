...
library IterableMapping {
    // Iterable mapping from address to PoolStaker
    struct Map {
        address[] keys;
...
contract StakingManagerV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // State variables
    IERC20 public stakeToken; // Token to be staked
    IERC20 public rewardToken; // Reward token
    uint256 public tokensStaked; // Total tokens staked
    uint256 public currentEpochReward; // Current epoch total reward
    uint256 public currentEpochStartTime; // Current epoch start time
    uint256 public currentEpochEndTime; // Current epoch end time

    uint16 public epochId; // Current epoch ID
    mapping(uint16 => mapping(address => UserInfoInEpoch)) public epochLog; // User reward info in epoch
    mapping(uint16 => EpochInfo) public epochInfos; // Epoch info

    IterableMapping.Map private poolStakers; // Staker info
    ...
    event EpochStarted(uint16 epochId, uint256 startTime, uint256 endTime, uint256 rewardAmount);
    ...
    function initialize(address _stakeTokenAddress, address owner, address _rewardTokenAddress) public initializer {
    ...
    // Unstake all tokens from existing pool
    function unstake() external nonReentrant {
        ...
        for (uint16 i = 0; i < epochId; i++) {
            if (epochInfos[i].endTime > staker.stakedTime && staker.stakedTime != 0) {
                UserInfoInEpoch storage userInfo = epochLog[i][msg.sender];
                rewardToken.safeTransfer(msg.sender, userInfo.lockedReward);
                userInfo.lockedReward = 0;
            }
        }
    ...
    // Deposit reward tokens for the current epoch
    ...
    // Get the number of rewards that user can get
    function getRewards(address _user) public view returns (uint256) {
        if (tokensStaked == 0) {
            return 0;
        }
        require(block.timestamp >= currentEpochStartTime && block.timestamp <= currentEpochEndTime, "Not within the current epoch");
        PoolStaker memory staker = poolStakers.get(_user);
        return staker.amount * (currentEpochEndTime - block.timestamp) / tokensStaked * currentEpochReward;
    }

    // Get the number of rewards that user has on a specific epoch
    ...
    function startEpoch(uint256 _startTime, uint256 _endTime, uint256 _rewardAmount) external onlyOwner {
        require(_startTime > currentEpochEndTime, "Epoch start time must be after the current epoch end time");
        require(_endTime > _startTime, "End time must be after start time");
        require(_rewardAmount > 0, "Reward amount can't be zero");
        
        rewardToken.safeTransferFrom(owner(), address(this), _rewardAmount);
        ...
        emit EpochStarted(epochId, _startTime, _endTime, _rewardAmount);
        epochId++;
    }
    ...
    function distribute() external onlyOwner {
        require(block.timestamp > currentEpochEndTime, "The epoch has not ended yet");
        
        uint256 count = poolStakers.size();
        for (uint256 i = 0; i < count; i++) {
            ...
            if (currentEpochStartTime < staker.stakedTime && staker.stakedTime < currentEpochEndTime) {
                ...
                UserInfoInEpoch storage userInfo = epochLog[epochId][key];
                userInfo.lockedReward += (staker.amount * currentEpochReward) / tokensStaked;
            }
        }
    }
    ...
    // Delete epoch data and distribute remaining rewards
    function deleteEpoch(uint16 _epoch) validEpoch(_epoch) external onlyOwner {
        uint256 count = poolStakers.size();
        for (uint256 i = 0; i < count; i++) {
            address stakerAddress = poolStakers.getKeyAtIndex(i);
            UserInfoInEpoch storage userInfo = epochLog[_epoch][stakerAddress];
            if (userInfo.lockedReward > 0) {
